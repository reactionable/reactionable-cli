	...
	implements CreateAppAdapter
{
	...
	constructor(
		...
		protected readonly _addUIFramework: AddUIFramework,
		@inject(new LazyServiceIdentifier(() => AddHostingIdentifier))
		protected readonly _addHosting: AddHosting,
		@inject(AddVersioning) protected readonly _addVersioning: AddVersioning,
		@inject(GenerateReadme) protected readonly _generateReadme: GenerateReadme,
		...
		protected readonly _createComponent: CreateComponent,
		@inject(CliService) protected readonly _cliService: CliService,
	) {
		super();
	}
	...
	get addVersioning(): AddVersioning {
		return this._addVersioning;
	}

	get generateReadme(): GenerateReadme {
	...
	get createComponent(): CreateComponent {
	...
	async addTsDevTools(realpath: string): Promise<void> {
		...
		await this.packageManagerService.execPackageManagerCmd(
			realpath,
			"ts-dev-tools install",
		);
		this.consoleService.success(
			`ts-dev-tools configuration has been installed`,
		);
	}
	...
	async addI18n(realpath: string): Promise<void> {
		...
		const entrypointFile =
			await this.fileFactory.fromFile<TypescriptFile>(entrypointPath);
	...
	async choosePackageManager(): Promise<PackageManagerType | undefined> {
		const availablePackageManagers =
			this.packageManagerService.getAvailablePackageManagers();
