...
describe("templateService", () => {
	...
	describe("renderTemplate", () => {
		it("should create a file from given namespace", async () => {
			...
			const expectedTranslationFile = resolve(
				testDirPath,
				"src/i18n/locales/en/common.json",
			);
			expect((await stat(expectedTranslationFile)).isFile()).toBe(true);
			expect(
				await readFile(expectedTranslationFile, "utf-8"),
			).toMatchSnapshot();
		});
		...
		it("should create a file from given namespace having nested config", async () => {
			...
			expect(await readFile(expectedConfigFile, "utf-8")).toMatchSnapshot(
				"TestEntitiesConfig.tsx",
			);

			const expectedTranslationFile = resolve(
				testDirPath,
				"src/i18n/locales/en/testEntities.json",
			);
			expect((await stat(expectedTranslationFile)).isFile()).toBe(true);
