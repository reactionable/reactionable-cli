...
import type { PackageManagerType } from "../PackageManagerService";
import type { IPackageManager, PackageJson } from "./IPackageManager";
...
> implements IPackageManager<PJ>
{
	...
	async getPackageJsonData<P extends keyof PJ = keyof PJ>(
		...
		const file = await this.fileFactory.fromFile<JsonFile>(
			packageJsonPath,
			encoding,
		);
