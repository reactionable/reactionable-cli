...
import {
	type ITypescriptImport,
	type ITypescriptImportModules,
	TypescriptImport,
} from "./TypescriptImport";
...
export class TypescriptFile extends StdFile {
	...
	getContent(): string {
		...
		const declarationsText = (this.declarations || [])
			.filter((value) => value.trim().length > 0)
			.join("");

		if (!importLines.length && declarationsText) {
			return declarationsText.trim();
		}
		if (importLines.length && !declarationsText) {
			return importLines.join("\n").trim();
		}
		if (importLines.length && declarationsText) {
			const declarationsTextWithoutLeadingNewline = declarationsText.replace(
				/^\n+/,
				"",
			);
			return `${importLines.join("\n")}\n\n${declarationsTextWithoutLeadingNewline}`.trim();
		}
		return "";
	}
	...
	protected parseContent(content: string): string {
		...
		let previousDeclarationKind: SyntaxKind | null = null;
		for (const bodyItem of body) {
			switch (bodyItem.kind) {
				case SyntaxKind.ImportDeclaration:
					this.parseImportDeclaration(bodyItem as ImportDeclaration);
					previousDeclarationKind = null;
					break;
				case SyntaxKind.EmptyStatement:
					break;
				default: {
					const declarationText = content.substring(
						bodyItem.getFullStart(),
						bodyItem.end,
					);
					const shouldAddLeadingBlankLine =
						previousDeclarationKind === SyntaxKind.ExpressionStatement &&
						bodyItem.kind === SyntaxKind.ExpressionStatement &&
						!declarationText.startsWith("\n\n");
					this.declarations.push(
						shouldAddLeadingBlankLine
							? `\n${declarationText}`
							: declarationText,
					);
					previousDeclarationKind = bodyItem.kind;
					break;
				}
			}
		}
		...
		try {
			...
			throw new Error(
				`An error occurred while parsing content "${content}", ${error}`,
				{
					cause: error,
				},
			);
	...
	protected parseImportDeclaration(bodyItem: ImportDeclaration): void {
		const moduleSpecifier = bodyItem.moduleSpecifier;
		const text =
			moduleSpecifier &&
			typeof moduleSpecifier === "object" &&
			"text" in moduleSpecifier
				? moduleSpecifier.text
				: undefined;
		const packageName = typeof text === "string" ? text : "";
	...
	protected sortImports(
		importA: TypescriptImport,
		importB: TypescriptImport,
	): number {
