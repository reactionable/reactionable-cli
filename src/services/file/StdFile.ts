...
export class StdFile {
	...
	async saveFile(
		...
		const parentDirExists =
			await this.directoryService.dirExists(parentDirPath);
		if (!parentDirExists) {
			throw new Error(
		...
		if (fileExists) {
			const fileContent = await this.fileService.getFileContent(
				filePath,
				encoding,
			);
			...
			const shouldPromptOverwrite =
				await this.fileDiffService.fileNeedsOverwrite(filePath, changes);

			if (shouldPromptOverwrite) {
				const overwrite = await this.cliService.promptOverwriteFileDiff(
					filePath,
					changes,
				);
